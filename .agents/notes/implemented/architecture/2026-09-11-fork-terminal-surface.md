...
Boundaries are deliberate and recorded in the package README: one Agent per invocation, a text-only composer with no attachments, folded Tool output, one choice per prompt for a `multiSelect` question, and the launcher-owned exit that every surface shares.
