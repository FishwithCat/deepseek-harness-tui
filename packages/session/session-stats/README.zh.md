...
每个字段在首个贡献事件之前均为 0；已装配的注册表恒提供该键，因此客户端读取值本身，而非键的存在性。客户端通过投影 seam 的快照与变更流渲染全日志数字；Web 聊天统计条是参考消费者，终端页脚则读取 `decodeMs`/`decodeTokens` 以显示其吞吐量数字。统计条的窗口折叠以相同字段名充当无单元时的回退。
