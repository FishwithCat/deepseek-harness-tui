...
在本目录运行 `pnpm build:ts` 构建入口、`pnpm build:native` 构建当前宿主声明的原生产物、`pnpm build:test-oracle` 构建独立的 flock 系统调用 fixture（测试前置数据）。随后用 `pnpm test` 验证入口、锁、打包及可用的内核行为。Linux 完整构建需要 musl-gcc；macOS 使用 `xcrun --sdk macosx cc` 选择配套的编译器与 SDK。根目录 `pnpm run build:native-system` 只构建源码测试所需的当前宿主 addon。
