...
describe('parseDshArgs', () => {
  ...
  it('boots the default terminal profile for a bare invocation, with an environment override', () => {
    ...
    // Removed launcher flags are no longer rejected by a missing profile: with
    // a default profile they belong to the app, which fails on them itself.
    ...
    process.env.DSH_DEFAULT_PROFILE = 'web'
    try {
      expect(parse([])).toEqual({ mode: 'profile', profile: 'web', patches: [], args: [] })
      // An empty override restores the upstream requirement.
      process.env.DSH_DEFAULT_PROFILE = ''
      expect(exitCode([])).toBe(1)
    } finally {
      delete process.env.DSH_DEFAULT_PROFILE
    }
  })

  it('rejects contradictory inputs', () => {
